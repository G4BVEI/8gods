<script>
	...
	const fragmentShader = `
        ...
        float intensity = length(finalCol);
        vec3 cyan = vec3(0.0, 1.0, 1.0);

        gl_FragColor = vec4(cyan * intensity * 3.0, 1.0);
	...
	onMount(() => {
		...
		// Initialize with actual canvas dimensions
		...
		// Use ResizeObserver to watch the canvas's container/size
